...
**Autonomous wireless certification testing, powered by 13 Gemini agents.**
...
[**Watch the 1-minute demo**](https://drive.google.com/drive/folders/1bsALqTXxQDTJe-uAerfoydPeAc_L3rAC?usp=sharing) 
...
> **Vision -> Text pipeline:** The webcam identification result is converted into a structured BOM (device name, chips, frequencies, power, form factor), which then feeds the 13-agent text pipeline. **One photo bootstraps the entire compliance workflow.**

## Why it's different

| Most "AI compliance" tools | LabPilot |
|---|---|
| Single LLM call producing a checklist | 13 Gemini agents orchestrated in parallel + sequential pipelines (10 calls fan out concurrently via `asyncio.gather`) |
...
2. **Create project + scope** -- Click the create button. The 13-agent text pipeline kicks off using the BOM that was generated from the photo. Watch agents stream events in real-time on the left panel. The intake agent's `device_profile` event tells the rest of the stack what kind of device it is.
...
- **AI:** Gemini 3.5 Flash (12 direct agents) + Antigravity Managed Agent (1 agent, `lp-report-assembler`, for sandboxed PDF assembly with Code Execution) = 13 agents total, Multimodal Vision (setup verification + device identification)
...
LapPilot/
  labpilot/
    backend/      # FastAPI + 13 Gemini agents (12 Flash + 1 Antigravity assembler)
