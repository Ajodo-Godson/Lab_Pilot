# LabPilot — Final Architectural Plan (V4)
...
| Must | Scan summary (simulated peak SAR + anomalies) passed into report agent | P1 + P3 |
...
P3 consumes the SSE stream, forwards each `sar_point` to the WebSocket monitor, and stores the final `scan_complete` summary for report generation.
...
Request:  { scan_summary: ScanSummary }
Response: { project_id: string, status: "reporting" }
...
// GET /api/simulator/stream   ← SSE, emits "sar_point" then "scan_complete"
...
async def start_scoping(pid: str):
    asyncio.create_task(run_project(pid, projects[pid], queues[pid]))
    return {"project_id": pid, "status": "scoping"}

@app.post("/api/projects/{pid}/report/generate")
async def generate_report(pid: str, body: dict):
    projects[pid]["scan_summary"] = body["scan_summary"]
    asyncio.create_task(run_report_project(pid, projects[pid], queues[pid]))
    return {"project_id": pid, "status": "reporting"}
...
async def run_project(pid: str, project: dict, q: asyncio.Queue):
    data = project["data"]
...
async def run_report_project(pid: str, project: dict, q: asyncio.Queue):
    model_name = os.getenv("GEMINI_MODEL")
    cache_name = os.getenv("CACHE_NAME")
    artifacts = project["artifacts"]
    scan_summary = project["scan_summary"]

    # --- 5 PARALLEL REPORT AGENTS (include simulated scan summary) ---
    ...
    await run_report_parallel(artifacts["test_plan"], artifacts["device_profile"], scan_summary,
...
SECTION_PROMPTS = {
    ...
    "report_measurement": "Summarize simulated measurement results. Peak SAR: {peak_sar} W/kg. Anomalies: {anomaly_count}. Test plan: {context}",
    "report_citer":       "For each simulated measurement value, cite the exact regulatory paragraph it would be evaluated against. Test plan: {context}",
...
- POST /api/projects/:id/scope: starts intake, jurisdiction, and test plan pipeline
- POST /api/projects/:id/report/generate: accepts {scan_summary: {peak_sar, anomaly_count, total_points, anomalies}}
  starts the report-agent pipeline
- GET /api/projects/:id/events: SSE stream of agent events
- GET /api/projects/:id/report: returns {pdf_base64, summary}

Scope pipeline runs in sequence:
...
Report pipeline runs after the simulated scan completes:
4. 5 parallel report agents (setup, measurement, citer, narrator, compliance):
   include simulated peak_sar and anomalies from scan_summary
...
| 3:30–4:30 | `/report/generate` endpoint + 5 parallel report agents using scan_summary |
| 4:30–5:30 | Full pipeline test: mock BOM → test plan → scan_summary → report_ready |
...
### Deliverables
1. SAR physics generator streaming simulated volumetric measurements over SSE
...
async def stream_sar_grid(params: dict, queue: asyncio.Queue):
    ...
    summary = {"peak_sar": 0, "anomaly_count": 0, "total_points": 0, "anomalies": []}
...
async def sim_stream():
    async def generate():
        while True:
            evt = await sim_queue.get()
            yield f"event: {evt['event']}\ndata: {json.dumps(evt['data'])}\n\n"
            if evt["event"] == "scan_complete":
                break
    return StreamingResponse(
        generate(), media_type="text/event-stream",
        headers={"Cache-Control": "no-cache", "X-Accel-Buffering": "no"})
...
async def handle_sar_monitor(ws: WebSocket):
    ...
    try:
        while True:
            ...
            # Keep the stream cheap: only call Flash near risk, at most once every 1.5s.
            should_call_flash = (
                point["pct_of_limit"] >= 0.75 and
                time.monotonic() - last_model_call >= 1.5
            )
            if not should_call_flash:
...
2. GET /api/simulator/stream — SSE stream:
   event: sar_point, data: {x, y, z, frequency_mhz, power_dbm, sar_w_kg, is_anomaly, pct_of_limit}
   event: scan_complete, data: {peak_sar, anomaly_count, total_points, anomalies}
...
SAR monitor: use rolling window of 8 points. Call Gemini only when pct_of_limit >= 0.75
and no more than once every 1.5 seconds.
...
SSE consumer — when scan is active, consume `/api/simulator/stream`:
- On `sar_point`, handle the point by doing BOTH:
...
- On `scan_complete`, store the final ScanSummary and call onScanComplete(summary)

Track scan summary in a ref:
const scanSummary = useRef({ peak_sar: 0, anomaly_count: 0, total_points: 0, anomalies: [] });

Update it on each anomaly point, then replace it with the simulator's final summary
on `scan_complete`. Expose getScanSummary() via useImperativeHandle
...
- Handle `sar_point` events by updating the 3D scene and forwarding the point to `/sar-monitor`
- Handle `scan_complete` by saving the final ScanSummary and showing "Generate report"
- "Generate report" button appears after `scan_complete`:
  On click: read getScanSummary() from SARViewport ref, POST to backend /api/projects/:id/report/generate
...
**[2:10–3:00]**
*"Five agents simultaneously draft the compliance report — including the simulated peak SAR and anomaly data from the scan."*
